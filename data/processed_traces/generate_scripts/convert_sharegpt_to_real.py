"""
Convert ShareGPT Vicuna dataset to Mooncake trace format with text prompts.
...
- prompt_text: raw human/user input text
- response_text: raw assistant/GPT output text
...
def count_tokens(text: str, encoder) -> int:
    ...
    if encoder is not None:
        # Disable special token checking to handle text like '<|endoftext|>' as normal text
...
def generate_block_hash_ids(
    ...
    # Use large offset to prevent collisions (each session gets 1M block range)
    base_offset = session_id * 100000  # Each session gets a different range
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Enable multi-turn mode: subsequent turns include accumulated context in "
